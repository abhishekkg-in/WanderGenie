    ...
    for item in data:
        ...
        try:
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?)
            ''', (item['Id'], item['destination'], item['description'], item['details'], item['metadata']))
...
def update_embeding(id, embedding):
    ...
    cursor.execute("UPDATE travel_knowledge_base SET embeding = ? WHERE id = ?", (serialized_embedding, id))
    cursor.connection.commit()
